...
public class 可移动木板_up_stop : MonoBehaviour
{
    ...
    void targetChange()
    {
        if (Mathf.Abs((transform.position - first_point.position).sqrMagnitude) < 0.1f)
        {
            stoptime = defaultStoptime;
            target = second_point.position;
        }
        else if (Mathf.Abs((transform.position - second_point.position).sqrMagnitude) < 0.1f)
        {
            stoptime = defaultStoptime;
